...
_build-bib $target_image $tag $type $config: (_rootful_load_image target_image tag)
    ...
    # uCore's own root fs choice (xfs). --rootfs=btrfs builds fine but the
    # resulting image hangs in GRUB right after "Probing EDD... ok" and never
    # reaches the kernel - reproduced on real Proxmox hardware.
    args+="--rootfs=xfs"
